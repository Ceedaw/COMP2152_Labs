...
#Loop through numbers with their indices
    for i in range(len(numbers)):
        # Calculate what number we need to reach the target
        needed = target - numbers[i]

#Check if the needed value exists in our dictionary
        if needed in seen:
            # Return the index of the needed value and current index
...
def shuffle_array(nums, n):
    ...
    #  Loop through range(n) and append alternating elements
    for i in range(n):
...
def first_unique_character(s):
    ...
    # Use for i in range(len(s)) to check each character
